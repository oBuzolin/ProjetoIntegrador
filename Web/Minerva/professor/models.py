...
class Minerva_Atividades(models.Model):
    id = models.AutoField(primary_key=True)
    titulo = models.CharField(max_length=100)
    escopo = models.TextField()
    matricula = models.ForeignKey(Minerva_Professor, null=True, blank=True, on_delete=models.SET_NULL)
    turma = models.ForeignKey(Minerva_Turma, null=True, blank=True, on_delete=models.SET_NULL)
    envio = models.DateTimeField(null=False)
    entrega = models.DateTimeField(null=True, default=None)
    nota = models.CharField(max_length=100, null=False)
...
class Minerva_Entrega(models.Model):
    id = models.AutoField(primary_key=True)
    aluno = models.ForeignKey(Minerva_Aluno, null=True, blank=True, on_delete=models.SET_NULL)  # Aluno que entregou
    atividade = models.ForeignKey(Minerva_Atividades, on_delete=models.CASCADE)
    arquivo = models.FileField(upload_to='entregas/')  # Arquivo enviado pelo aluno
    ...
    def entregou_no_prazo(self):
        return self.data_entrega <= self.atividade.entrega
