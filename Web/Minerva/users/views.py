        ...
        try:
            ...
            if professor.senha == senha_hash:  # Verifica a senha
                ...
                if login_entry.status == 'ativo':
                    # Redireciona para a página do professor com base no ID (matrícula)
                    return redirect('professor_home', matricula=professor.matricula)
                else:
...
def handle_login(user_id, user_type, request):
    login_entry = Minerva_Login.objects.get(user_type_id=user_id)
    if login_entry.status == 'ativo':
        next_url = request.GET.get('next', f'{user_type}_home')
        return redirect(next_url)
    else:
        messages.error(request, 'Conta inativa ou bloqueada.')
        return render(request, 'login.html')
